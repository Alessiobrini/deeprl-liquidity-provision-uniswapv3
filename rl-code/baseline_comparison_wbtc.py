"""
WBTC/USDC 0.3% Pool Baseline Comparison Script
Enhanced baseline comparison for WBTC pool with statistical analysis.
Runs PPO (with Optuna hyperparameter optimization) and baseline strategies, then generates comprehensive statistical comparisons.

Supports parallel execution across multiple GPUs via command-line arguments.
"""
...
def main():
    """Main execution function for WBTC pool."""
    # Parse command-line arguments for parallel execution
    parser = argparse.ArgumentParser(description='WBTC Baseline Comparison with optional parallel execution')
    parser.add_argument('--start-window', type=int, default=None, help='Start window index (for parallel execution)')
    parser.add_argument('--end-window', type=int, default=None, help='End window index (for parallel execution)')
    parser.add_argument('--gpu-id', type=int, default=None, help='GPU ID (for parallel execution)')
    ...
    # Modify output directory based on GPU ID if specified
    if args.gpu_id is not None:
        output_dir = os.path.join(base_dir, "output", f"baseline_comparison_wbtc_gpu{args.gpu_id}")
        print(f"\n[Parallel Mode] GPU {args.gpu_id}: Processing windows {args.start_window} to {args.end_window}")
    else:
        output_dir = os.path.join(base_dir, "output", "baseline_comparison_wbtc")

    os.makedirs(output_dir, exist_ok=True)

    # Load WBTC hyperparameters
    config_file = "pool_wbtc_usdc_030.yaml"
    with open(os.path.join(config_dir, config_file), "r") as f:
        params = yaml.safe_load(f)

    print(f"\n{'='*70}")
    print(f"WBTC/USDC 0.3% Pool Baseline Comparison")
    print(f"{'='*70}")
    print(f"Config: {config_file}")
    print(f"Fee tier: {params['delta']}%")
    print(f"Action values: {params['action_values']}")
    print(f"Initial BTC: {params['x']}")
    ...
    print(f"Loaded {len(prices):,} hours of WBTC/USDC price data")
    ...
    # Define baseline configurations (adapted for 60-tick spacing)
    # WBTC pool has 60 tick spacing vs WETH's 10, so we scale width candidates accordingly
    baseline_configs = [
        ("PassiveWidthSweep",
         PassiveWidthSweep,
         {"width_candidates": list(range(60, 601, 60)),  # 60-600 ticks (1x-10x base spacing)
        ...
        ("ReactiveRecentering",
         ...
         {"vol_thresholds": [0.005, 0.01, 0.02],
          ...
          "width_ticks": params["action_values"][1] if len(params["action_values"]) > 1 else 120,
          "deposit_action_idx": 1}),
    ]
    ...
    # Determine window range based on arguments
    total_windows = len(windows) - 5
    if args.start_window is not None and args.end_window is not None:
        start_idx = args.start_window
        end_idx = min(args.end_window, total_windows - 1)
        window_range = range(start_idx, end_idx + 1)
        print(f"  - Window range: {start_idx} to {end_idx} (subset of {total_windows} total windows)")
    else:
        window_range = range(total_windows)
        start_idx = 0
        end_idx = total_windows - 1
        print(f"  - Window range: All {total_windows} windows")
    ...
    print(f"  - Baselines: Parameter sweeps (optimized for 60-tick spacing)")
    print(f"  - Total windows to process: {len(window_range)}\n")

    # Iterate through rolling windows
    for window_idx in window_range:
        print(f"\n{'='*60}")
    ...
    print(f"\n\n{'='*60}")
    if args.gpu_id is not None:
        print(f"GPU {args.gpu_id} STATISTICS (Windows {start_idx}-{end_idx})")
    else:
        print("OVERALL STATISTICS ACROSS ALL WINDOWS (WBTC/USDC)")
    print(f"{'='*60}\n")

    overall_stats = compute_statistics(all_results, baseline_key='PPO')
    print(overall_stats.to_string(index=False))

    # Save overall results (with GPU ID suffix if parallel)
    if args.gpu_id is not None:
        overall_stats_file = f"gpu{args.gpu_id}_statistics.csv"
        detailed_results_file = f"gpu{args.gpu_id}_detailed_results.csv"
    else:
        overall_stats_file = "overall_statistics.csv"
        detailed_results_file = "detailed_results_all_windows.csv"

    overall_stats.to_csv(
        os.path.join(output_dir, overall_stats_file),
        index=False
    )
    ...
    detailed_results.to_csv(
        os.path.join(output_dir, detailed_results_file),
        index=False
    )
